...
| 4 | **Kannada review by a native speaker.** All Kannada in `src/i18n/ui.ts`, `content/site.json`, `content/programs/*`, and every record's `title`/`summary` was written during this rebuild, not by the association | Published, and needs review |
...
| 6 | **CMS OAuth worker** — deploy `sveltia/sveltia-cms-auth`, set `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, `ALLOWED_DOMAINS=jnvckm.org,www.jnvckm.org,jnvckm.pages.dev`, create the GitHub OAuth app with callback `<worker-url>/callback`, then set `backend.base_url` in `public/admin/config.yml` | `base_url` is commented out; browser editing does not work yet. Local editing does |
